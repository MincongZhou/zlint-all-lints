...
用法:
    python3 run_zlint.py <证书目录> [输出目录] [--timeout 秒] [--jsonl]
...
def run_batch(cert_dir, out_dir=None, timeout=None, jsonl=False):
    ...
    if not os.path.isdir(cert_dir):
        print(f"错误: 证书目录不存在 -> {cert_dir}", file=sys.stderr)
        sys.exit(1)
...
def interactive():
    """无参数时的交互式输入：证书目录必填，其余可回车跳过"""
    print("=== 交互模式（直接回车使用默认值，输入 q 退出）===")

    # 证书目录：必填，循环直到存在（支持 ~ 展开）
    cert_dir = os.path.expanduser(input("证书目录: ").strip())
    while True:
        ...
        cert_dir = os.path.expanduser(input("请重新输入证书目录 (q 退出): ").strip())
...
def main():
    ...
    if not cert_dir:
        print("用法: python3 run_zlint.py <证书目录> [输出目录] [--timeout 秒] [--jsonl]")
        sys.exit(1)
